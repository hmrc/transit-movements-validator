...
class V2BusinessValidationServiceSpec extends AnyFreeSpec with Matchers with ScalaFutures with ScalaCheckDrivenPropertyChecks with TestActorSystem {
  ...
  "Json" - {
    ...
    "when we validate the message recipient" - {
      ...
      "when messageRecipient is invalid" in {
        ...
        val sut            = new V2BusinessValidationServiceImpl(createConfig())
    ...
    "when we validate the LRN" - {
      "when the LRN is 'invalid' and the LRN validation is disabled, no errors are returned" in {
        ...
        val sut            = new V2BusinessValidationServiceImpl(createConfig(lrnValidationEnabled = false))
        val (preMat, flow) = sut.businessValidationFlow(MessageType.DeclarationData, MessageFormat.Json)
      ...
      "when the LRN is 'invalid' and the LRN validation is enabled, an error is returned" in {
        ...
        val sut            = new V2BusinessValidationServiceImpl(createConfig(lrnValidationRegex = "^[a-zA-Z]{1,35}$"))
        val (preMat, flow) = sut.businessValidationFlow(MessageType.DeclarationData, MessageFormat.Json)
