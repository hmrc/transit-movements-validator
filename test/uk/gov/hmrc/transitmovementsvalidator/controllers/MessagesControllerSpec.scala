  ...
  "when APIVersion is 2.1" - {
    ...
    "On validate XML" - {
      ...
      "on a valid XML file for a request or response type where response types are enabled, return No Content" in forAll(Gen.oneOf(MessageType.values)) {
        messageType =>
          ...
          when(mockConfig.validateRequestTypesOnly).thenReturn(false)
      ...
      "on an invalid XML file, return Ok with a list of errors" in {
        ...
        when(mockV2XmlValidationService.validate(eqTo(validCode), any[Source[ByteString, ?]])(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => EitherT.leftT[Future, Unit](ValidationError.XmlFailedValidation(errorList))
          )
        when(mockV2BusinessValidationService.businessValidationFlow(eqTo(validCode), eqTo(MessageFormat.Xml))(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => (EitherT.rightT[Future, ValidationError](()), Flow[ByteString])
          )
        val messagesController =
    ...
    "On validate JSON" - {
      ...
      "on a valid JSON file, with the application/json content type, return No Content" in {
        when(mockV2JsonValidationService.validate(eqTo(validCode), any[Source[ByteString, ?]])(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => EitherT.rightT[Future, ValidationError](())
          )
        when(mockV2BusinessValidationService.businessValidationFlow(eqTo(validCode), eqTo(MessageFormat.Json))(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => (EitherT.rightT[Future, ValidationError](()), Flow[ByteString])
          )
        val messagesController =
      ...
      "with no content type header, must return Unsupported Media Type" in {
        ...
        val source  = Source.single(ByteString(validJson, StandardCharsets.UTF_8))
        val request = FakeRequest("POST", s"/messages/$validCode/validate/", FakeHeaders(Seq("APIVersion" -> "2.1")), source)
        val result  = messagesController.validate(validCode.code)(request)
...
//        val errorList = NonEmptyList(XmlSchemaValidationError(1, 1, "text1"), List(XmlSchemaValidationError(2, 2, "text2")))
//
//        when(mockV3XmlValidationService.validate(eqTo(validCode), any[Source[ByteString, ?]])(any[Materializer], any[ExecutionContext]))
//          .thenAnswer(
//            _ => EitherT.leftT[Future, Unit](ValidationError.XmlFailedValidation(errorList))
//          )
//        when(mockV3BusinessValidationService.businessValidationFlow(eqTo(validCode), eqTo(MessageFormat.Xml))(any[Materializer], any[ExecutionContext]))
//          .thenAnswer(
//            _ => (EitherT.rightT[Future, ValidationError](()), Flow[ByteString])
//          )
//        val messagesController =
...
//
//      "on a valid JSON file, with the application/json content type, return No Content" in {
//        when(mockV3JsonValidationService.validate(eqTo(validCode), any[Source[ByteString, ?]])(any[Materializer], any[ExecutionContext]))
//          .thenAnswer(
//            _ => EitherT.rightT[Future, ValidationError](())
//          )
//        when(mockV3BusinessValidationService.businessValidationFlow(eqTo(validCode), eqTo(MessageFormat.Json))(any[Materializer], any[ExecutionContext]))
//          .thenAnswer(
//            _ => (EitherT.rightT[Future, ValidationError](()), Flow[ByteString])
//          )
//        val messagesController =
