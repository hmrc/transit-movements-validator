...
class MessagesControllerSpec
  ...
  val v3ValidCode: MessageType = MessageType.DeclarationData
  ...
  "when APIVersion is 3.0" - {
    "On validate XML" - {
      ...
      "on a valid XML file, with the application/xml content type, return No Content" in forAll(Gen.oneOf(MessageType.requestValues)) {
        messageType =>
          ...
          when(
            mockBusinessValidationService.businessValidationFlow(eqTo(messageType), eqTo(MessageFormat.Xml))(any[Materializer], any[ExecutionContext])
          )
            .thenAnswer(
      ...
      "on an invalid XML file, return Ok with a list of errors" in {
        ...
        when(mockXmlValidationService.validate(eqTo(v3ValidCode), any[Source[ByteString, ?]])(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => EitherT.leftT[Future, Unit](ValidationError.XmlFailedValidation(errorList))
          )
        when(mockBusinessValidationService.businessValidationFlow(eqTo(v3ValidCode), eqTo(MessageFormat.Xml))(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => (EitherT.rightT[Future, ValidationError](()), Flow[ByteString])
          )
        val messagesController =
    ...
    "On validate JSON" - {
      ...
      "on a valid JSON file, with the application/json content type, return No Content" in {
        when(mockJsonValidationService.validate(eqTo(v3ValidCode), any[Source[ByteString, ?]])(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => EitherT.rightT[Future, ValidationError](())
          )
        when(mockBusinessValidationService.businessValidationFlow(eqTo(v3ValidCode), eqTo(MessageFormat.Json))(any[Materializer], any[ExecutionContext]))
          .thenAnswer(
            _ => (EitherT.rightT[Future, ValidationError](()), Flow[ByteString])
          )
        val messagesController =
